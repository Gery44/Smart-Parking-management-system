...
const AdminParkingSlots = () => {
  ...
  const handleOpenDialog = (mode, slot = null) => {
    ...
    setSelectedSlot(slot);
    if (mode === 'create') {
      setSlotData({ slotNumber: '', status: 'AVAILABLE', hourlyRate: '' });
    } else if (mode === 'edit' && slot) {
      setSlotData({
        slotNumber: slot.slotNumber,
        status: slot.status,
  ...
  const handleSubmit = async () => {
    try {
      ...
      let message = '';
      if (dialogMode === 'create') {
        await axios.post('/api/parking-slots', slotData);
        message = 'Parking slot created successfully!';
      } else if (dialogMode === 'edit') {
        await axios.put(`/api/parking-slots/${selectedSlot.id}`, slotData);
        message = 'Parking slot updated successfully!';
      } else if (dialogMode === 'delete') {
        await axios.delete(`/api/parking-slots/${selectedSlot.id}`);
        message = 'Parking slot deleted successfully!';
  ...
  const filteredSlots = parkingSlots.filter(slot => {
    return (
      slot.slotNumber?.toString().toLowerCase().includes(searchTerm) ||
      slot.status?.toLowerCase().includes(searchTerm) ||
      slot.hourlyRate?.toString().toLowerCase().includes(searchTerm) ||
      slot.bookedBy?.toLowerCase().includes(searchTerm) ||
      slot.startTime?.toLowerCase().includes(searchTerm) ||
      slot.endTime?.toLowerCase().includes(searchTerm)
    );
  });
